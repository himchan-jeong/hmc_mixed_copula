...
// 3: Gumbel
// 4: Frank
real dCphi_du1du2(real u1, real u2, real phi, int copula) {
  real c;
  if (copula==4) { // Frank
...
real copulapdf_log(real[] y1, real[] z, vector mu1, vector mu2, real sigma1, real phi, int n, int copula){
  ...
  logl = 0.0;
  for (i in 1:n){
    s = log(dCphi_du1du2(normal_cdf(y1[i] , mu1[i], sigma1), 
...
// 
// parameters {
//   real phi_trans; // copula parmeter, transformed to real line
...
//   real phi; // copula parameter, which we'll "untransform"
//   for (i in 1:n) {
//     v[i] = sign[i] * abs_z[i];
//   }
//   mu1 = x1 * b1;
//   mu2 = x2 * b2;
//   // untransform phi_trans
//   if (copula == 1) { //Gaussian
//     phi = tanh(phi_trans);
//   } else if (copula == 2) { //Clayton
//     phi = exp(phi_trans); // Assuming range of Clayton is (0, inf)
//   } else if (copula == 3) { //Gumbel
//     phi = 1 + exp(phi_trans);
//   } else if (copula == 4) { //Frank
//     phi = phi_trans;
//   } else {
//     reject("parmeter copula should be an integer between 1 and 4.")
//   }
// }
// 
// model {
...
//   phi_trans ~ normal(0, 10); //prior on transformed phi, easier this way
//   increment_log_prob(copulapdf_log(y1, v, mu1, mu2, sigma1, phi, n, copula));
// }
// ********************************************************************************************
...
model {
  ...
  b2 ~ normal(0, 100);
  sigma1 ~ cauchy(0, 5); //half-Cauchy implicit
  phi_trans ~ normal(0, sigma_phitrans); //prior on transformed phi, easier this way
